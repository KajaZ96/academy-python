...
class TestPunkAPI(unittest.TestCase):
    ...
    def test_defult_beers(self):
        res_body = requests.get(self.base_url).json()
        self.assertEqual(1, res_body[0]["id"])  # sprawdzenie, czy pierwszy ID = 1
        self.assertEqual(25, len(res_body))  # sprawdzenie długości listy, czy lista = 25

    def test_beer_123(self):
        requests.get(self.base_url + "/123")

    def test_40_elements_4th_page(self):
        params = {
            "page": 4,  # parametry w słowniku, zamiast dokładnego url --> bazowy url + parametry ze słowmika
            "per_page": 40
        }
